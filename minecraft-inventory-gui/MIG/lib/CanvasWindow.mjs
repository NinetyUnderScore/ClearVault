...
export class CanvasWindow extends SimpleEmitter {
  ...
  drawItem (obj, x, y) {
    ...
    const offset = 0.125 * (1-(String(obj.count).length-1))
    this.drawText({ value: String(obj.count).padStart(2," ") ?? '', stroke: false, fontStyle: '8px MinecraftiaRegular', style: '#373737' }, x + 6 - offset, y + 17)
    this.drawText({ value: String(obj.count).padStart(2," ") ?? '', stroke: false, fontStyle: '8px MinecraftiaRegular', style: 'white' }, x + 5 - offset, y + 16)
  }
  ...
  drawText (obj, x, y) {
    ...
    y *= this.scale
    this.drawCtx.save()
    this.drawCtx.font = obj.fontStyle ?? 'normal normal 11px MinecraftiaRegular'
  ...
  drawBox (aroundBB) {
    this.drawCtx.fillStyle = '#FFFFFF80'
  ...
  drawInput (obj, [x, y, mx], text) {
    ...
    this.drawCtx.font = obj.fontStyle ?? 'normal normal 8px MinecraftiaRegular'
